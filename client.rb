...
class Client
  class << self
    ...
    def pnd_time_based(amount, buy_price, wait_seconds, market) # buys, waits time, sells
      ...
      # make buy order
      log "Putting buy order for #{fts currencies_to_buy} #{market} at #{fts buy_price} BTC, took #{Time.now - start_time} ms"
      ...
      if get_order(order["uuid"])["Closed"].nil?
        ...
        log "Could not buy, canceling order"
      else
        # make sell order, take profit
        current_price = currency_price(market)
        log "Putting sell order for #{fts currencies_to_buy} #{currency} at #{fts current_price} BTC"
        order = market_sell_limit(market, currencies_to_buy, current_price)
        log "Order id is #{order["uuid"]}"
        ...
        reward = currencies_to_buy * current_price * 0.9975 # 0.0025% fee
        profit = reward - cost
    ...
    def pnd(amount, buy_when, sell_when, currency, recursive = false)
      ...
      start_time = Time.now
      amount = 0.01 # BTC
      currency_to_buy = "BTC-#{currency}" # currency
